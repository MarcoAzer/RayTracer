...
MeshConsolidator::MeshConsolidator(
    ...
    for(const ObjFilePath & objFile : objFileList) {
        if( addUV )
            ObjFileDecoder::decode(objFile.c_str(), meshId, positions, normals, uv);
        else
...
vector<vec3> MeshConsolidator::getVertexPositionData() const{
	return m_vertexPositionData;
}

vector<vec2> MeshConsolidator::getVertexUVData() const{
    return m_vertexUV;
}
